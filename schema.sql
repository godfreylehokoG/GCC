...
CREATE TABLE event_registrations (
  ...
  attendee_type text DEFAULT 'primary',
  attendee_index integer DEFAULT 0,
  attendee_label text,
  attendee_details_provided boolean DEFAULT true,
  booker_first_name text,
  booker_last_name text,
  booker_email text,
  booker_phone text,
  guest_count integer DEFAULT 0,
  guest_details_collected boolean DEFAULT false,
  primary_contact_name text,
  title text,
  organisation text,
  rsvp_total_guests integer,
  rsvp_guest_details text,
  vip_status text,
  gala_seating_selection text,
  airport_transport_required text,
  local_transport_required text,
